...
import { ChevronLeft, ChevronRight, Users, Phone, Check, X, DollarSign, AlertTriangle, Pencil, Save, ListChecks, UserX } from "lucide-react"
...
export default function MarksPage() {
  ...
  const [bulkLessonId, setBulkLessonId] = useState<string | null>(null)
  const [bulkDate, setBulkDate] = useState<Date | null>(null)
  const [bulkAttendanceMap, setBulkAttendanceMap] = useState<Record<string, boolean>>({})
  ...
  // ---------- Bulk Attendance ----------
  const openBulkAttendance = (lessonId: string, date: Date) => {
    const map: Record<string, boolean> = {}
    groupStudents.forEach(s => {
      const existing = attendanceData[s.id]?.[lessonId]
      map[s.id] = existing ? existing.present : true
    })
  ...
  const toggleBulkStudent = (studentId: string) => {
    setBulkAttendanceMap(prev => ({ ...prev, [studentId]: !prev[studentId] }))
  }

  const markAllPresent = () => {
    const allPresent: Record<string, boolean> = {}
    groupStudents.forEach(s => { allPresent[s.id] = true })
    setBulkAttendanceMap(allPresent)
  }

  const markAllAbsent = () => {
    const allAbsent: Record<string, boolean> = {}
    groupStudents.forEach(s => { allAbsent[s.id] = false })
    setBulkAttendanceMap(allAbsent)
  }
  ...
  const handleBulkConfirm = async () => {
    ...
    try {
      const attendance = Object.entries(bulkAttendanceMap).map(([studentId, isPresent]) => ({
        student_id: Number(studentId),
        is_present: isPresent,
        reason: null,
      }))
      await api.postAttendance({ lesson_id: Number(bulkLessonId), attendance })
      setAttendanceData(prev => {
        const next = { ...prev }
        Object.entries(bulkAttendanceMap).forEach(([studentId, isPresent]) => {
          if (!next[studentId]) next[studentId] = {}
          next[studentId][bulkLessonId] = { present: isPresent, reason: null }
        })
        return next
      })
  ...
  return (
    <div className="p-4 bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
            ...
            <thead className="bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200">
              <tr>
                ...
                {dates.map((dateObj, index) => (
                  <th key={index} className="px-1 py-2 text-center align-top">
                    <DateHeader date={dateObj.date} unitNumber={mode === "homework" ? dateObj.unitNumber : undefined} />
                    {mode === "attendance" && teacherRole === 'main' && !isFuture(dateObj.date) && (
                      <button
                        type="button"
                        onClick={() => openBulkAttendance(dateObj.lessonId, dateObj.date)}
                        className="mt-1.5 w-full inline-flex items-center justify-center gap-1 text-[10px] sm:text-xs px-2 py-1 sm:px-3 sm:py-1.5 rounded-lg bg-blue-500 text-white hover:bg-blue-600 border border-blue-400 font-semibold transition-all shadow-sm active:scale-95"
                      >
      ...
      <Dialog open={bulkModalOpen} onOpenChange={(open) => { if (!open) setBulkModalOpen(false) }}>
        <DialogContent className="bg-white w-[95vw] max-w-lg max-h-[90vh] overflow-y-auto p-0 gap-0">
          <DialogHeader className="p-4 pb-2 border-b border-gray-100">
            <div className="flex items-center justify-between">
              <div>
                ...
                <DialogDescription className="text-[10px] text-gray-400 mt-0.5">
                  {bulkDate ? format(bulkDate, "dd MMMM yyyy") : ''} — Faqat yo'q o'quvchilarni belgilang
                </DialogDescription>
              </div>
              <div className="flex items-center gap-1">
                <button
                  ...
                  className="text-[9px] px-2 py-1 rounded-md bg-green-50 text-green-600 hover:bg-green-100 border border-green-200 font-medium transition-colors"
                >
                  Hammasi bor
                </button>
                <button
                  ...
                  className="text-[9px] px-2 py-1 rounded-md bg-red-50 text-red-600 hover:bg-red-100 border border-red-200 font-medium transition-colors"
                >
                  Hammasi yo'q
                </button>
              </div>
          ...
          <div className="overflow-y-auto max-h-[50vh] p-2">
            {groupStudents.length === 0 ? (
              <div className="text-center py-8 text-xs text-gray-400">O'quvchilar yo'q</div>
            ) : (
              <div className="space-y-1">
                {groupStudents.map((student) => {
                  const isPresent = bulkAttendanceMap[student.id]
                  return (
                    <button
                      key={student.id}
                      type="button"
                      onClick={() => toggleBulkStudent(student.id)}
                      className={`w-full flex items-center gap-3 p-2.5 rounded-xl border transition-all duration-150 active:scale-[0.98] ${
                        isPresent === false
                          ? 'bg-red-50 border-red-200 shadow-sm'
                          : 'bg-white border-gray-100 hover:border-gray-200 hover:bg-gray-50'
                      }`}
                    >
                      <Avatar className="w-8 h-8 ring-2 shrink-0">
                        <AvatarImage src={getStudentPhotoUrl(student.photo)} alt={student.first_name} />
                        <AvatarFallback className="text-[9px] bg-gradient-to-br from-blue-500 to-indigo-600 text-white font-bold">
                          {student.first_name?.[0]}{student.last_name?.[0]}
                        </AvatarFallback>
                      </Avatar>
                      ...
                      <div className={`shrink-0 text-[10px] font-bold px-2.5 py-1 rounded-full transition-all ${
                        isPresent === false
                          ? 'bg-red-100 text-red-700'
                          : 'bg-green-100 text-green-700'
                      }`}>
                        {isPresent === false ? 'Yo\'q' : 'Bor'}
                      </div>
                    </button>
                  )
                })}
          ...
          <div className="p-3 border-t border-gray-100 bg-gray-50/50">
            <div className="flex items-center justify-between mb-2">
              <div className="text-[10px] text-gray-500">
                {Object.values(bulkAttendanceMap).filter(v => v === false).length} ta o'quvchi yo'q
              </div>
              <div className="text-[10px] text-gray-500">
                {Object.values(bulkAttendanceMap).filter(v => v === true).length} ta o'quvchi bor
              </div>
            </div>
            <div className="flex gap-2">
              ...
              <Button
                size="sm"
                className="flex-1 rounded-xl text-xs h-9 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white shadow-md"
              ...
              >
                {isSavingBulk ? (
                  ...
                  `Tasdiqlash (${Object.values(bulkAttendanceMap).filter(v => v === false).length} yo'q)`
                )}
              </Button>
            </div>
          </div>
